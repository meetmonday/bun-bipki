...
export const economyComposer = new Composer()
	...
	.command("transfer", async (context) => {
		if (!context.args)
		...
		try {
			await transfer(context.from.id, toId, amount, "bipki");
			return context.send(
	...
	.command("transfer_mega", async (context) => {
		if (!context.args)
			return context.send("Использование: /transfer_mega <id> <сумма>");
		...
		try {
			await transfer(context.from.id, toId, amount, "megabipki");
			return context.send(
				format`✅ Переведено ${amount} 💎 мегабипок пользователю ${toId}`,
			);
		} catch (error) {
			return context.send(
				error instanceof Error ? error.message : "Ошибка перевода",
