...
export const greetingScene = new Scene("greeting")
	...
	.step("message", (context) => {
		if (context.scene.step.firstTime) return context.send("How old are you?");

		const age = Number(context.text);
		if (!context.text || Number.isNaN(age))
	...
	.step("message", async (context) => {
		await context.send(
			`Nice to meet you! I now know that your name is ${context.scene.state.name} and you are ${context.scene.state.age} years old.`,
		);
