...
const authPlugin = new Plugin("auth").derive(
...
const orderScene = new Scene("order")
	...
	.step("callback_query", async (ctx, next) => {
		...
		await ctx.editText(
			format`Site: ${bold(result.data.site)}\n\nNow send the address:`,
		);
		await ctx.answer();
	...
	.step("message", async (ctx) => {
		...
		await ctx.send(
			format`Ordered ${bold(site!)} → ${link(address!, "https://example.com")}`,
		);
...
const bot = new Bot(process.env.BOT_TOKEN!)
	...
	.callbackQuery(nav, async (ctx) => {
		...
		switch (ctx.queryData.to) {
			case "home":
				return ctx.editText("Home screen");
			case "history":
				return ctx.editText("History (empty)");
			case "order":
