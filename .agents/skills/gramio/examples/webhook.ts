...
const bot = new Bot(process.env.BOT_TOKEN as string).command(
	"start",
	(context) => context.send("Hello from webhook!"),
);

new Elysia().post("/webhook", webhookHandler(bot, "elysia")).listen(3000);
...
const preConfigured = new Bot(process.env.BOT_TOKEN as string).command(
	"start",
