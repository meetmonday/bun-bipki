...
function parseExports(content) {
	...
	const reDirect =
		/export\s+(?:declare\s+)?(?:function|class|const|let|var|interface|type|enum)\s+(\w+)/g;
...
function parseTopLevelFunctions(content) {
	...
	while ((m = re.exec(content)) !== null) {
		...
		const fullSig = content.slice(m.index + (jsdoc?.length ?? 0), endIdx + 1);
		fns.push({
			name,
			jsdoc,
...
async function findPluginDir(name) {
	const candidates = [`@gramio/${name}`, `gramio-${name}`, name];
...
if (!query && !showAll) {
	console.error(`Usage: node ${process.argv[1]} [--list] <plugin-name>`);
	console.error();
	console.error(
	...
	console.error("  --list         List all installed @gramio/* packages");
	console.error();
...
async function runOnDir(dir, pkg) {
	...
	const fns = parseTopLevelFunctions(content).filter((f) =>
		exports.has(f.name),
	);
