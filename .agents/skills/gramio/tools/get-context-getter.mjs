...
if (!query && !showAll) {
	...
	console.error();
	console.error("Inspect @gramio/contexts classes, mixins, and wrapper types.");
	console.error(
...
if (searchMode) {
	...
	if (hits.length === 0) {
		...
		console.log(`${partial.length} members partially matching "${query}":`);
		console.log();
...
function printSection(title, items) {
	...
	for (const m of items) {
		...
		console.log(`  ${m.name}${m.summary ? " — " + m.summary : ""}${viaNote}`);
	}
	console.log();
}
